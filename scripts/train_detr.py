...
print("=" * 50)
print("🚗 DETR - ТЕСТИРОВАНИЕ НА ДАННЫХ UA-DETRAC")
print("=" * 50)
...
# Параметры
BATCH_SIZE = 1  # DETR требует много памяти
CONFIDENCE_THRESHOLD = 0.5

DATA_DIR = "data"
os.makedirs("models/detr", exist_ok=True)
...
print("\n📦 ЗАГРУЗКА МОДЕЛИ DETR...")
print("⏳ Это может занять несколько минут (загрузка весов)...")
...
print(f"✅ Модель DETR загружена (предобучена на COCO)")
...
# Тестирование модели
print("\n📊 ТЕСТИРОВАНИЕ DETR НА ВАЛИДАЦИОННЫХ ДАННЫХ...")
...
with torch.no_grad():
    for batch_idx, (images, targets) in enumerate(val_loader):
        # DETR требует особой предобработки через processor
        try:
            # Конвертируем тензоры в PIL изображения для процессора
            from PIL import Image
            import io
            
            # Берем первое изображение из батча (batch_size=1)
            img_tensor = images[0]
            img_np = (img_tensor.permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8)
            img_pil = Image.fromarray(img_np)
            
            # Предобработка через processor
            ...
            total_vehicles_found += len(vehicle_indices)
            total_detections += len(boxes)
            
        except Exception as e:
            print(f"   ⚠️ Ошибка на батче {batch_idx}: {e}")
            continue
        
        if (batch_idx + 1) % 5 == 0:
            print(f"   Обработано {batch_idx + 1}/{len(val_loader)} батчей")
            print(f"   Найдено машин: {total_vehicles_found}")

avg_inference_time = total_inference_time / num_batches if num_batches > 0 else 0

print("\n" + "=" * 50)
print(f"📊 РЕЗУЛЬТАТЫ ТЕСТИРОВАНИЯ DETR:")
...
print("=" * 50)
print("✅ DETR тестирование завершено!")